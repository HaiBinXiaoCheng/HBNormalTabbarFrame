...
- (void)doRequestPostImageWithUrlString:(NSString *)urlString
    ...
    [self doRequestPostImagesWithUrlString:urlString paramsDic:paramsDic formImageArr:formImageArr imageMaxKBSize:maxKBSize successBlock:successBlock failBlock:failBlock timeOutBlock:timeoutBlock];
}
/**
 上传多张图片，多张图片的名字都是一样的，avatar
...
- (void)doRequestPostImagesWithUrlString:(NSString *)urlString
                paramsDic:(NSDictionary *)paramsDic
              formImageArr:(NSArray<UIImage *> *)formImages imageMaxKBSize:(NSInteger)maxKBSize
             successBlock:(void(^)(NSURLSessionDataTask *operation, id resultData))successBlock
                failBlock:(void(^)(NSURLSessionDataTask *operation, NSError *error))failBlock
                            timeOutBlock:(void(^)(BOOL isLoginSuccess))timeoutBlock {
        
    [self doRequestPostImagesWithUrlString:urlString paramsDic:paramsDic formImageArr:formImages FileNameArr:nil imageMaxKBSize:maxKBSize successBlock:successBlock failBlock:failBlock timeOutBlock:timeoutBlock];
}
/**
 上传多张图片,并且每张图片的名字不一样
...
- (BOOL)requestSuccessWithResultData:(id)resultData withMsg:(NSString **)msgStr {
    if (!resultData || ![resultData isDictionaryValid]) {
        *msgStr = @"服务器错误";
        return NO;
    }
